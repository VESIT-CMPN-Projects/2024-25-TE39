"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.modelPrediction = void 0;
const axios_1 = __importDefault(require("axios"));
// Placeholder function to call the Flask API for plant disease prediction
const modelPrediction = (imageUrl) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const response = yield axios_1.default.post("http://127.0.0.1:5000/predict_plant_disease", { imageUrl });
        // Explicitly type response.data
        const result = response.data;
        return result.predicted_disease;
    }
    catch (error) {
        throw new Error("Error calling the Flask model API");
    }
});
exports.modelPrediction = modelPrediction;
